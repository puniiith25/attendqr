...
import { useSignalEffect, useComputed, signal, effect, computed, batch } from '@preact/signals';
...
function MonthViewWeek({ week }) {
    ...
    return (jsx(Fragment, { children: jsx("div", { "data-testid": DATE_PICKER_WEEK, className: "sx__date-picker__week", children: weekDays.map((weekDay) => (jsx("button", { type: "button", tabIndex: hasFocus(weekDay) ? 0 : -1, disabled: !isDateSelectable(weekDay.day), "aria-label": getLocalizedDate$1($app.datePickerState.datePickerDate.value, $app.config.locale.value), className: weekDay.classes.join(' '), "data-focus": hasFocus(weekDay) ? 'true' : undefined, onClick: () => selectDate(weekDay.day), onKeyDown: handleKeyDown, children: weekDay.day.day }))) }) }));
}
...
function ViewSelection() {
    ...
    const [selectedViewLabel, setSelectedViewLabel] = useState(getInitialSelectedViewLabel());
    useSignalEffect(() => {
        const selectedView = $app.config.views.value.find((view) => view.name === $app.calendarState.view.value);
        if (!selectedView)
            return;
    ...
    return (jsxs("div", { className: `sx__view-selection ${isOpen ? 'is-open' : ''}`, children: [jsx("label", { for: viewSelectId, id: viewLabelId, className: "sx__view-selection-label", children: $app.translate('View') }), jsxs("div", { id: viewSelectId, tabIndex: 0, role: "button", "aria-describedby": viewLabelId, "aria-label": $app.translate('Select View'), className: "sx__view-selection-selected-item sx__ripple", onClick: () => setIsOpen(!isOpen), onKeyDown: handleSelectedViewKeyDown, children: [selectedViewLabel, jsx("img", { className: "sx__view-selection-chevron", src: img, alt: "" })] }), isOpen && (jsx("ul", { "data-testid": "view-selection-items", className: "sx__view-selection-items", children: availableViews.map((view) => (jsx("li", { "aria-label": $app.translate('Select View') + ' ' + $app.translate(view.label), tabIndex: -1, role: "button", onKeyDown: (keyboardEvent) => navigateUpOrDown(keyboardEvent, view.name), onClick: () => handleClickOnSelectionItem(view.name), className: 'sx__view-selection-item' +
...
function ForwardBackwardNavigation() {
    ...
    const [rangeEndMinusOneRange, setRangeEndMinusOneRange] = useState(null);
    const [rangeStartPlusOneRange, setRangeStartPlusOneRange] = useState(null);
    useSignalEffect(() => {
        const selectedView = $app.config.views.value.find((view) => view.name === $app.calendarState.view.value);
        if (!selectedView)
            return;
    ...
    return (jsx(Fragment, { children: jsxs("div", { className: "sx__forward-backward-navigation", "aria-label": localizedRange, "aria-live": "polite", children: [jsx(Chevron, { disabled: !!($app.config.minDate.value &&
                        dateFromDateTime((_a = rangeEndMinusOneRange === null || rangeEndMinusOneRange === void 0 ? void 0 : rangeEndMinusOneRange.toString()) !== null && _a !== void 0 ? _a : '') <
                            $app.config.minDate.value.toString()), onClick: () => navigate('backwards'), direction: 'previous', buttonText: $app.translate('Previous period') }), jsx(Chevron, { disabled: !!($app.config.maxDate.value &&
...
function TimeGridEvent({ calendarEvent, dayBoundariesDateTime, isCopy, setMouseDown, }) {
    ...
    return (jsxs(Fragment, { children: [jsx("div", { ref: eventRef, id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, "data-event-id": calendarEvent.id, onClick: handleOnClick, onDblClick: handleOnDoubleClick, onKeyDown: handleKeyDown, onMouseDown: handlePointerDown, onMouseUp: handlePointerUp, onTouchStart: handlePointerDown, onTouchEnd: handlePointerUp, className: classNames.join(' '), tabIndex: 0, role: "button", style: {
                ...
                }, children: jsxs("div", { "data-ccid": customComponentId.current, className: "sx__time-grid-event-inner", children: [!customComponent && !hasCustomContent && (jsxs(Fragment$1, { children: [isCompact && calendarEvent.title && (jsxs("div", { className: "sx__title-and-time-compact", children: [jsx("div", { className: "sx__time-grid-event-title", children: calendarEvent.title }), jsx("div", { className: "sx__time-grid-event-time", children: timeFn(calendarEvent.start, $app.config.locale.value) })] })), !isCompact && calendarEvent.title && (jsx("div", { className: "sx__time-grid-event-title", children: calendarEvent.title })), (!isCompact || (isCompact && !calendarEvent.title)) && (jsxs("div", { className: "sx__time-grid-event-time", children: [jsx(TimeIcon, { strokeColor: eventCSSVariables.iconStroke }), getEventTime(calendarEvent.start, calendarEvent.end)] })), calendarEvent.people && calendarEvent.people.length > 0 && (jsxs("div", { className: "sx__time-grid-event-people", children: [jsx(UserIcon, { strokeColor: eventCSSVariables.iconStroke }), concatenatePeople(calendarEvent.people)] })), calendarEvent.location && (jsxs("div", { className: "sx__time-grid-event-location", children: [jsx(LocationPinIcon, { strokeColor: eventCSSVariables.iconStroke }), calendarEvent.location] }))] })), hasCustomContent && (jsx("div", { dangerouslySetInnerHTML: {
                                __html: ((_c = calendarEvent._customContent) === null || _c === void 0 ? void 0 : _c.timeGrid) || '',
                            } })), $app.config.plugins.resize &&
                            !((_d = calendarEvent._options) === null || _d === void 0 ? void 0 : _d.disableResize) && (jsx("div", { className: 'sx__time-grid-event-resize-handle', onMouseDown: startResize, onTouchStart: startResize }))] }) }), eventCopy && (jsx(TimeGridEvent, { calendarEvent: eventCopy, isCopy: true, setMouseDown: setMouseDown, dayBoundariesDateTime: dayBoundariesDateTime }))] }));
}
...
function TimeGridDay({ calendarEvents, date, backgroundEvents, }) {
    ...
    return (jsxs("div", { className: classNames.value.join(' '), "data-time-grid-date": toDateString$1(date), onClick: (e) => handleOnClick(e, $app.config.callbacks.onClickDateTime), onDblClick: (e) => handleOnClick(e, $app.config.callbacks.onDoubleClickDateTime), "aria-label": getLocalizedDate$1(date, $app.config.locale.value), onMouseLeave: () => setMouseDownOnChild(false), onMouseUp: handlePointerUp, onTouchEnd: handlePointerUp, onMouseDown: handleMouseDown, children: [backgroundEvents.map((event) => (jsx(Fragment, { children: jsx(TimeGridBackgroundEvent, { backgroundEvent: event, date: date.toString() }) }))), eventsWithConcurrency.map((event) => (jsx(TimeGridEvent, { calendarEvent: event, dayBoundariesDateTime: dayBoundariesDateTime, setMouseDown: setMouseDownOnChild }, event.id)))] }));
}
...
function DateGridEvent({ calendarEvent, gridRow, isCopy, }) {
    ...
    const customComponentId = useRef(customComponent
    ...
    return (jsxs(Fragment, { children: [jsxs("div", { id: isCopy ? getTimeGridEventCopyElementId(calendarEvent.id) : undefined, tabIndex: 0, "aria-label": calendarEvent.title +
                    ...
                    ...getBorderRadius(hasOverflowLeft, hasOverflowRight, !!customComponent),
                }, children: [!customComponent && !hasCustomContent && (jsxs(Fragment$1, { children: [hasOverflowLeft && (jsx("div", { className: 'sx__date-grid-event--left-overflow', style: overflowStyles })), jsxs("span", { className: "sx__date-grid-event-text", children: [calendarEvent.title, " \u00A0", calendarEvent.start instanceof Temporal.ZonedDateTime && (jsx("span", { className: "sx__date-grid-event-time", children: timeFn(calendarEvent.start, $app.config.locale.value) }))] }), hasOverflowRight && (jsx("div", { className: 'sx__date-grid-event--right-overflow', style: overflowStyles }))] })), hasCustomContent && (jsx("div", { dangerouslySetInnerHTML: {
                        ...
                        !endsAfterWeek && (jsx("div", { className: "sx__date-grid-event-resize-handle", onMouseDown: startResize, onTouchStart: startResize }))] }), eventCopy && (jsx(DateGridEvent, { calendarEvent: eventCopy, gridRow: gridRow, isCopy: true }))] }));
}
...
function DateGridDay({ calendarEvents, date, backgroundEvents, }) {
    ...
    return (jsxs("div", { className: "sx__date-grid-day", "data-date-grid-date": date, children: [fullDayBackgroundEvent && (jsx("div", { className: "sx__date-grid-background-event", title: fullDayBackgroundEvent.title, style: {
                ...
                return (jsx(DateGridEvent, { calendarEvent: event, gridRow: index + 1 }, event.start.toString() + event.end.toString() + index));
...
const WeekWrapper = ({ $app, id }) => {
    ...
    return (jsx(Fragment, { children: jsx(AppContext.Provider, { value: $app, children: jsxs("div", { className: "sx__week-wrapper", id: id, children: [jsx("div", { className: "sx__week-header", children: jsxs("div", { className: "sx__week-header-content", children: [jsx(DateAxis, { week: Object.values(week.value).map((day) => {
                                    ...
                                    }) }), jsx("div", { className: "sx__date-grid", "aria-label": $app.translate('Full day- and multiple day events'), children: Object.values(week.value).map((day) => (jsx(DateGridDay, { date: day.date, calendarEvents: day.dateGridEvents, backgroundEvents: day.backgroundEvents }, day.date))) }), jsx("div", { className: "sx__week-header-border" })] }) }), jsxs("div", { className: "sx__week-grid", children: [jsx(TimeAxis, {}), Object.values(week.value).map((day) => {
                                ...
                                return (jsx(TimeGridDay, { calendarEvents: day.timeGridEvents, backgroundEvents: day.backgroundEvents, date: zonedDateTime }, day.date));
                            })] })] }) }) }));
};
...
function CalendarHeader() {
    ...
    return (jsx("header", { className: 'sx__calendar-header', "data-ccid": headerContentId, children: !headerContent && (jsxs(Fragment, { children: [jsxs("div", { className: 'sx__calendar-header-content', children: [headerContentLeftPrependId && (jsx("div", { "data-ccid": headerContentLeftPrependId })), jsx(TodayButton, {}), jsx(ForwardBackwardNavigation, {}), jsx(RangeHeading, {}, $app.config.locale.value), $app.config.showWeekNumbers.value && isDayOrWeekView && (jsx(WeekNumber, {})), headerContentLeftAppendId && (jsx("div", { "data-ccid": headerContentLeftAppendId }))] }), jsxs("div", { className: 'sx__calendar-header-content', children: [headerContentRightPrependId && (jsx("div", { "data-ccid": headerContentRightPrependId })), $app.config.plugins.timezoneSelect &&
                            $app.config.plugins.timezoneSelect.isEnabled.value && (jsx($app.config.plugins.timezoneSelect.ComponentFn, { "$app": $app })), $app.config.views.value.length > 1 && (jsx(ViewSelection, {}, keyForRerenderingOnLocaleChange + '-view-selection')), jsx(AppWrapper, { "$app": datePickerAppSingleton }), headerContentRightAppendId && (jsx("div", { "data-ccid": headerContentRightAppendId }))] })] })) }));
}
...
function MonthGridEvent({ gridRow, calendarEvent, date, isFirstWeek, isLastWeek, }) {
    ...
    return (jsxs("div", { draggable: !!$app.config.plugins.dragAndDrop, "data-event-id": calendarEvent.id, "data-ccid": customComponentId.current, onMouseDown: (e) => createDragStartTimeout(handleStartDrag, e), onMouseUp: (e) => setClickedEventIfNotDragging(calendarEvent, e), onTouchStart: (e) => createDragStartTimeout(handleStartDrag, e), onTouchEnd: (e) => setClickedEventIfNotDragging(calendarEvent, e), onClick: handleOnClick, onDblClick: handleOnDoubleClick, onKeyDown: handleKeyDown, className: classNames.join(' '), style: {
            ...
            backgroundColor: customComponent
                ? undefined
                : eventCSSVariables.backgroundColor,
        }, tabIndex: 0, role: "button", children: [!customComponent && !hasCustomContent && (jsxs(Fragment, { children: [calendarEvent.start instanceof Temporal.ZonedDateTime && (jsx("div", { className: "sx__month-grid-event-time", children: timeFn(calendarEvent.start, $app.config.locale.value) })), jsx("div", { className: "sx__month-grid-event-title", children: calendarEvent.title })] })), hasCustomContent && (jsx("div", { dangerouslySetInnerHTML: {
...
function MonthGridDay({ day, isFirstWeek, isLastWeek }) {
    ...
    return (jsxs("div", { className: wrapperClasses.join(' '), "data-date": toDateString$1(day.date), onClick: (e) => $app.config.callbacks.onClickDate &&
            $app.config.callbacks.onClickDate(day.date, e), "aria-label": getLocalizedDate$1(day.date, $app.config.locale.value), onDblClick: (e) => { var _a, _b; return (_b = (_a = $app.config.callbacks).onDoubleClickDate) === null || _b === void 0 ? void 0 : _b.call(_a, day.date, e); }, onMouseDown: handleMouseDown, children: [fullDayBackgroundEvent && (jsx(Fragment, { children: jsx("div", { className: "sx__month-grid-background-event", title: fullDayBackgroundEvent.title, style: {
                        ...fullDayBackgroundEvent.style,
                    } }) })), jsxs("div", { className: "sx__month-grid-day__header", children: [isFirstWeek ? (jsx(Fragment, { children: monthGridDayNameCustomComponent ? (jsx("div", { "data-ccid": monthGridDayNameCCID })) : (jsx("div", { className: "sx__month-grid-day__header-day-name", children: getDayNameShort(dayDate, $app.config.locale.value) })) })) : null, monthGridDateCCID ? (jsx("div", { "data-ccid": monthGridDateCCID })) : (jsx("div", { className: dateClassNames.join(' '), children: dayDate.day }))] }), jsx("div", { className: "sx__month-grid-day__events", children: Object.values(day.events)
                    ...
                    return (jsx(MonthGridEvent, { gridRow: index + 1, calendarEvent: event, date: day.date.toString(), isFirstWeek: isFirstWeek, isLastWeek: isLastWeek }));
                }) }), numberOfNonDisplayedEvents > 0 ? (jsx("button", { type: "button", className: "sx__month-grid-day__events-more sx__ripple--wide", "aria-label": getAriaLabelSingularOrPlural(numberOfNonDisplayedEvents), onClick: handleClickAdditionalEvents, children: getEventTranslationSingularOrPlural(numberOfNonDisplayedEvents) })) : null] }));
}

function MonthGridWeek({ week, isFirstWeek, isLastWeek, }) {
    const $app = useContext(AppContext);
    return (jsxs("div", { className: "sx__month-grid-week", children: [$app.config.showWeekNumbers.value && (jsx("div", { className: "sx__month-grid-week__week-number", children: getWeekNumber(week[0].date, $app.config.firstDayOfWeek.value) })), week.map((day) => {
                /**
                 * The day component keeps internal state, and needs to be thrown away once the day changes.
                 * */
                return (jsx(MonthGridDay, { day: day, isFirstWeek: isFirstWeek, isLastWeek: isLastWeek }, toDateString$1(day.date)));
            })] }));
}
...
function MonthAgendaEvent({ calendarEvent }) {
    ...
    if (wasEventAddedInLastSecond(calendarEvent))
        classNames.push('is-event-new');
    return (jsxs("div", { className: classNames.join(' '), "data-ccid": customComponentId.current, "data-event-id": calendarEvent.id, style: {
            backgroundColor: customComponent
                ? undefined
                : eventCSSVariables.backgroundColor,
            color: customComponent ? undefined : eventCSSVariables.color,
...
const ListWrapper = ({ $app, id, }) => {
    ...
    const renderEventTimes = (event, dayDate) => {
        ...
        if (!isMultiDay) {
            return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-start-time", children: startZDT.toLocaleString($app.config.locale.value, timeOptions) }), event.end && (jsx("div", { className: "sx__list-event-end-time", children: endZDT.toLocaleString($app.config.locale.value, timeOptions) }))] }));
        }
        if (isFirstDay) {
            return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-start-time", children: startZDT.toLocaleString($app.config.locale.value, timeOptions) }), jsx("div", { className: "sx__list-event-arrow", children: "\u2192" })] }));
        }
        if (isLastDay) {
            return (jsxs(Fragment, { children: [jsx("div", { className: "sx__list-event-arrow", children: "\u2190" }), jsx("div", { className: "sx__list-event-end-time", children: endZDT.toLocaleString($app.config.locale.value, timeOptions) })] }));
        }
        return jsx("div", { className: "sx__list-event-arrow", children: "\u2194" });
    };
    ...
    return (jsx(AppContext.Provider, { value: $app, children: jsx("div", { id: id, className: "sx__list-wrapper", ref: wrapperRef, children: daysWithEvents.length === 0 ? (jsx("div", { className: "sx__list-no-events", children: $app.translate('No events') })) : (daysWithEvents.map((day) => (jsxs("div", { className: "sx__list-day", "data-date": day.date, children: [jsx("div", { className: "sx__list-day-header", children: jsx("div", { className: "sx__list-day-date", children: toJSDate(day.date).toLocaleDateString($app.config.locale.value, {
                                weekday: 'long',
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric',
                            }) }) }), jsx("div", { className: "sx__list-day-events", children: day.events.map((event) => (jsxs("div", { className: "sx__event sx__list-event", onClick: (e) => handleEventClick(e, event), onDblClick: (e) => handleEventDoubleClick(e, event), onKeyDown: (e) => handleEventKeyDown(e, event), tabIndex: 0, role: "button", children: [jsx("div", { className: `sx__list-event-color-line`, style: {
                                        backgroundColor: `var(--sx-color-${event._color})`,
                                    } }), jsxs("div", { className: "sx__list-event-content", children: [jsx("div", { className: "sx__list-event-title", children: event.title }), jsx("div", { className: "sx__list-event-times", children: renderEventTimes(event, day.date) })] })] }, event.id))) }), jsx("div", { className: "sx__list-day-margin" })] }, day.date)))) }) }));
};
...
export { AppContext, CalendarApp, DateGridDay, TimeAxis, TimeGridDay, createCalendar, createPreactView, createViewDay, createViewList, createViewMonthAgenda, createViewMonthGrid, createViewWeek, externalEventToInternal, setRangeForDay, setRangeForMonth, setRangeForWeek, sortEventsForWeekView, toDateString$1 as toDateString, toDateTimeString, toJSDate, toTimeString, viewDay, viewList, viewMonthAgenda, viewMonthGrid, viewWeek };
